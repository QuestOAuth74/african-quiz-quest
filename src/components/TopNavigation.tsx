...
const TopNavigation = () => {
  ...
  return (
    <nav className="w-full bg-primary backdrop-blur-lg border-b border-white/10 py-2 sm:py-3 px-3 sm:px-4 fixed top-0 z-40 transition-all duration-300">
      <div className="max-w-7xl mx-auto flex items-center justify-between">
        <div className="flex items-center gap-2 animate-fade-in">
          <span className="text-white font-bold text-base sm:text-lg truncate transition-colors duration-300">
            Historia Africana
          </span>
        </div>
        ...
        <div className="hidden md:flex items-center gap-1 animate-slide-in-right">
          <Link
            to="/"
            className="text-white/90 hover:text-white hover:bg-white/10 transition-colors px-3 py-2 text-sm font-medium rounded-md"
          >
            Home
          </Link>
        ...
        <div className="md:hidden">
          <Sheet open={open} onOpenChange={setOpen}>
            ...
            <SheetContent
              side="right"
              className="w-80 bg-primary border-white/10"
            >
              <div className="flex flex-col h-full">
                ...
                <nav className="flex flex-col space-y-2 pt-6">
                  <Link
                    ...
                    className="flex items-center gap-3 text-white/90 hover:text-white transition-colors p-3 rounded-md hover:bg-white/10"
                  >
                    <span>Home</span>
                  </Link>
                  ...
                  <button
                    ...
                    className="flex items-center gap-3 text-white/90 hover:text-white transition-colors p-3 rounded-md hover:bg-white/10 w-full text-left"
                  >
                ...
                <div className="mt-auto pt-6 border-t border-white/10">
                  {isAuthenticated ? (
                    <button
                      onClick={handleSignOut}
                      className="flex items-center justify-center gap-2 text-primary bg-white hover:bg-white/90 transition-colors p-3 rounded-md font-medium w-full"
                    >
                      <span>Sign Out</span>
                    </button>
                  ) : (
                    <Link
                      to="/auth"
                      onClick={() => setOpen(false)}
                      className="flex items-center justify-center gap-2 text-primary bg-white hover:bg-white/90 transition-colors p-3 rounded-md font-medium"
                    >
